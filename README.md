...
We developed a Blockchain Ledger to track the open source components used across a supply chain. The ledger enables the establishment of trust on who did what, when and how within the supply chain  That is, which suppliers created which software parts, included which open source and who prepared which compliance artifacts. This is particular helpful for product manufactures who build products that utilizing software from multiple different suppliers. This is also particularly challenging because a mechanism is needed to maintain global state information about suppliers and parts  across the supply chain in  order to achieve accountability. We solved this problem by using Linux Foundation’s Hyperledger project's Sawtooth platform to construct a blockchain Ledger that tracks which suppliers delivered which software parts that used which open source for which products and who delivered (or did not deliver) which open source compliance artifacts  (e.g., source code, legal notices, SPDX data, cryptography data) . We needed to define a new data construct , the **Compliance Envelope**, which represents a standard method of collecting, indexing and bundling the compliance artifacts  such that they can be delivered as a single unit for a given software part. We use the Ledger to maintain the associate between the supplier, part and the corresponding compliance envelope.
